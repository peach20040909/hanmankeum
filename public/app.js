...
const go = view => { location.hash = `#/team/${state.team.id}/${view}`; };
...
function newTeamModal() {
  const memberRow = () => `<div class="member-row"><input class="input" name="m-name" placeholder="이름" maxlength="20"><input class="input" name="m-role" placeholder="역할 (예: 개발)" maxlength="30"><input class="input gh" name="m-github" placeholder="GitHub 아이디" maxlength="40"><button type="button" class="icon-button" data-action="row-remove" aria-label="팀원 삭제">${icon('close', 'sm')}</button></div>`;
  newTeamModal.row = memberRow;
  openModal('새 팀 프로젝트', 'NEW TEAM / 착수 설정', `<form id="team-form" class="form-grid"><label class="field">과목명<input class="input" name="course" required maxlength="60" placeholder="캡스톤디자인(2)"></label><label class="field">팀 이름<input class="input" name="name" required maxlength="30" placeholder="팀 3"></label><label class="field full">프로젝트명<input class="input" name="project" required maxlength="60" placeholder="캠퍼스 분실물 매칭 앱"></label><label class="field">시작일<input class="input" type="date" name="start_date"></label><label class="field">마감일<input class="input" type="date" name="deadline" required></label><label class="field full">GitHub 저장소 (선택)<input class="input" name="repo" placeholder="owner/repository" pattern="[\\w.\\-]+/[\\w.\\-]+"></label><label class="field full">기여 유형 템플릿<select class="select-control" name="preset">${PRESETS.map((p, i) => `<option value="${i}">${p.title} — ${p.categories.map((c, j) => `${c} ${p.weights[j]}`).join(' · ')}</option>`).join('')}</select></label><div class="full"><div class="field" style="margin-bottom:8px">팀원 (GitHub 아이디로 기록을 연결합니다)</div><div id="member-rows">${memberRow() + memberRow() + memberRow()}</div><button type="button" class="button small" data-action="row-add">팀원 추가</button></div></form>`, '가중치는 만든 뒤 착수 설정에서 팀과 함께 조정할 수 있습니다.', true, `<div class="flex gap8"><button class="button small" data-action="close-modal">취소</button><button class="button primary small" data-action="create-team">팀 만들기</button></div>`);
}
...
document.addEventListener('click', e => {
  ...
  switch (b.dataset.action) {
    ...
    case 'create-team': run(async () => {
      ...
      const names = fd.getAll('m-name'), roles = fd.getAll('m-role'), ghs = fd.getAll('m-github');
      const team = await api('/teams', { method: 'POST', body: {
        ...
        repo: fd.get('repo').trim(), categories: p.categories, weights: p.weights,
        members: names.map((n, i) => ({ name: n, role: roles[i], github: ghs[i] })).filter(m => m.name.trim()),
      } });
